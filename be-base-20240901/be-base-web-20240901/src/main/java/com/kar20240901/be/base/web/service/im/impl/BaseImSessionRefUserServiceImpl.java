...
public class BaseImSessionRefUserServiceImpl extends ServiceImpl<BaseImSessionRefUserMapper, BaseImSessionRefUserDO>
    ...
    public Map<Long, BaseImSessionRefUserQueryLastContentVO> queryLastContentMap(NotEmptyIdSet dto) {
        ...
        Map<Long, BaseImSessionRefUserQueryLastContentVO> queryLastContentMap =
            baseImSessionRefUserQueryLastContentVoList.stream()
                .collect(Collectors.toMap(BaseImSessionRefUserQueryLastContentVO::getSessionId, it -> it));

        Map<Long, BaseImSessionRefUserQueryLastContentVO> resMap = MapUtil.newHashMap(sessionIdList.size());

        for (BaseImSessionRefUserQueryLastContentVO item : unReadCountList) {

            BaseImSessionRefUserQueryLastContentVO baseImSessionRefUserQueryLastContentVO =
                queryLastContentMap.get(item.getSessionId());

            if (baseImSessionRefUserQueryLastContentVO == null) {

                item.setLastContent("");
                item.setLastContentType(BaseImSessionContentTypeEnum.NONE.getCode());
                item.setLastContentCreateTs(TempConstant.NEGATIVE_ONE);
