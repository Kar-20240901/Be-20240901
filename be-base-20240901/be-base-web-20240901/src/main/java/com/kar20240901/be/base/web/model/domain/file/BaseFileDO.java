...
public class BaseFileDO extends TempEntityTree<BaseFileDO> {
    ...
    @TableField(
        value = "(CASE WHEN type = 101 THEN NULL WHEN type = 201 THEN (SELECT COALESCE(SUM(file_size),0) FROM base_file subA WHERE subA.pid_path_str LIKE CONCAT('%|', base_file.id, '|%')) ELSE NULL END)",
