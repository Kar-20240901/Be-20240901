...
public class SysRequestController {
    ...
    public R<Page<SysRequestDO>> myPage(@RequestBody @Valid SysRequestPageDTO dto) {
        return R.okData(baseService.myPage(dto));
    }

    @PostMapping("/allAvgPro")
    @Operation(summary = "所有请求的平均耗时-增强：增加筛选项")
    public R<SysRequestAllAvgVO> allAvgPro(@RequestBody @Valid SysRequestPageDTO dto) {
        return R.okData(baseService.allAvgPro(dto));
    }

    @PostMapping("/self/loginRecord")
    @Operation(summary = "当前用户：登录记录")
    public R<Page<SysRequestDO>> selfLoginRecord(@RequestBody @Valid SysRequestSelfLoginRecordPageDTO dto) {
